...
const RestaurantCard: React.FC<RestaurantCardProps> = ({ restaurant }) => {
  ...
  const prevImage = () => {
    ...
    setModalState(prev => ({
      ...prev,
      imageIndex: prev.imageIndex === 0 ? currentDish.images.length - 1 : prev.imageIndex - 1
    }));
  };
  ...
  return (
    <>
      ...
      <div className="grid md:grid-cols-2 gap-0">
        ...
        <div className="p-8">
          <div className="mb-6">
            <h3 className="text-3xl font-bold text-slate-900 mb-2">{restaurant.name}</h3>
          ...
          <div className="border-t border-slate-200 pt-6">
            <h4 className="text-xl font-semibold text-slate-900 mb-4">Menu dégustation</h4>
                ...
                <div key={index} className="flex justify-between items-center py-3 border-b border-slate-100 last:border-b-0">
                  <div className="flex items-center flex-1">
                    ...
                    {dish.images.length > 0 && (
                      ...
                      >
                        <Camera size={16} />
                        <span className="text-sm">({dish.images.length})</span>
                      </button>
                    )}
                  </div>
                  <span className="font-semibold text-slate-900 ml-4">{dish.price}€</span>
                </div>
              ))}
            </div>
